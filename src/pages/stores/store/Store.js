...
import { useFetchStores, useDeleteStore, useEditStore } from '../../../hooks';

const Store = () => {
  const [details, setDetails] = useState([]);
  const [modal, setModal] = useState(false);
console.log("we are here")
  const { data, status } = useFetchStores();
  const [deleteStore] = useDeleteStore()
  const [createStore] = useEditStore()
      ...
      <CDataTable
        ...
        scopedSlots={{
          ...
          show_view_icon: (item) => {
            return (
              <td className="py-2">
                <Link to={`/stores/${item.id}/items`}>
                  <CIcon
                    size={'xl'}
                    className="text-info"
                    title="view store Items"
          ...
          details: (item, index) => <CCollapse show={details.includes(index)}>
                ...
                >
                  ...
                  <CButton onClick={() => deleteStore(item.id)} size="sm" color="danger" className="ml-1">
                    Delete
                  </CButton>
                </CCardBody>
              </CCollapse>,
